...
use munge_sys::{munge_decode, munge_encode, munge_free, munge_strerror, MungeErr};
use std::{
    borrow::{Cow, ToOwned},
    convert::TryInto,
    ffi::{CStr, CString},
...
impl Credential {
    #[inline]
    pub fn uid(self: &'_ Self) -> u32 {
        self.uid
    }

    #[inline]
    pub fn gid(self: &'_ Self) -> u32 {
        self.gid
    }

    #[inline]
    pub fn payload(self: &'_ Self) -> Option<&'_ [u8]> {
...
impl MungeError {
    pub fn with_str<R, F>(self: &'_ Self, f: F) -> R
    where
        F: FnOnce(Cow<str>) -> R,
    {
        let errno = self.to_number();
        let slice = unsafe { CStr::from_ptr(munge_strerror(errno)) };
        f(slice.to_string_lossy())
    }

    #[inline]
    pub fn to_string(self: &'_ Self) -> String {
        self.with_str(|s| s.into_owned())
    }

    pub fn to_number(self: &'_ Self) -> MungeErr::Type {
        match *self {
    ...
    pub fn try_from_number(number: MungeErr::Type) -> Option<Self> {
        Some(match number {
...
impl error::Error for Error {
    fn source(self: &'_ Self) -> Option<&'_ (dyn error::Error + 'static)> {
...
pub fn encode(payload: Option<&'_ [u8]>) -> Result<String, Error> {
    let mut cred: *const c_char = std::ptr::null();
    let (payload_ptr, payload_len) = if let Some(payload) = payload {
        (
            payload.as_ptr() as *const c_void,
            payload.len().try_into().expect("payload length overflow"),
        )
    } else {
        (ptr::null(), 0)
    };

    let result = unsafe { munge_encode(&mut cred, std::ptr::null_mut(), payload_ptr, payload_len) };
    ...
    if result != MungeErr::SUCCESS {
        return Err(MungeError::try_from_number(result).unwrap().into());
    } else {
...
pub fn decode(cred: impl Into<String> + AsRef<str>) -> Result<Credential, Error> {
    ...
    let cred: &CStr = match cred.as_ref().bytes().position(|x| x == b'\0') {
        None => {
            ...
            bytes.push(b'\0');
            buffer = unsafe { CString::from_vec_unchecked(bytes) };
            &*buffer
        }
